...
class RobertaHeterogeneousGraph(RobertaBase):
    ...
    def forward(self, samples):
        ...
        if "erc_embeddings" in samples:
            # Direct precomputed emotion embeddings provided (graph space expected)
            erc_embeddings = samples["erc_embeddings"].to(self.device)
            erc_logits = samples.get("erc_logits")
            if erc_logits is not None:
                erc_logits = erc_logits.to(self.device)
        else:
            if self.lightmode:
