...
class StructuredDialogueDiscourseParser:
    def __init__(self,
        ...
        self.model = Model(encoder_config, encoder=encoder, link_only=False, encode_batch_size=parse_bs)
        ...
        print(f"[SDDP] Device: {self.device} | encode_batch_size: {parse_bs} | max_num_contexts: {self.max_num_contexts}")
    ...
    def _preprocess(self, batch):
        ...
        for sample in batch:
            ...
            # Clamp number of contexts to fixed maximum
            if len(res) > self.max_num_contexts:
                res = res[:self.max_num_contexts]
            p_sample['length'] = min(len(res), self.max_num_contexts)
